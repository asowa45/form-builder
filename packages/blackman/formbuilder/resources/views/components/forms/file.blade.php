<div class="col-md-{{$form_field->column_size}}">
    <div class="form-group">
        ...
        @if($editable == 0 && ($form_data != null && empty($form_data['for_empty_request_id'])))
            ...
            @endphp
            @if($attachLink != null)
                <a href="{{url('form-builder/download?filePath='.$attachLink)}}" class="btn btn-primary square">
                    <i class="ft-download"></i> Download File
                </a>
            @elseif($attachLink == null)
...
{{--            @if(\Auth::user()->ability('client|webmaster','download-repository-files'))--}}
            <a href="{{url('form-builder/download?filePath='.$attachLink)}}" class="btn btn-primary btn-sm square mr-1 mb-1">
                <i class="ft-download"></i> {{$form_data[$form_field->name]}}
            </a>
            <a class="btn btn-danger btn-sm mb-1" href="{{url('form-builder/set-field-null?table='.$tableName.'&id='.$id.'&col='.$form_field->name)}}"
